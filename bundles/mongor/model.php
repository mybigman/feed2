...
class Model {
	...
	{
		$count = $this->connection->count(static::$collection);
	...
	public function _save($options = array())
	{
		...
		if ($this->exists)
		{
			$update = $this->connection->update(static::$collection, array('_id' => $this->attributes['_id']), array('$set' => $this->dirty), $options);
			
			$result =  $update ? $this->attributes : false;
		}
		else
		{
			$result = $this->connection->insert(static::$collection, $this->attributes, $options);
		}
	 ...
	 * @return mixed
	 */
	public function has_one_or_many($model, $foreign_key)
	{
		$this->relating = __FUNCTION__;
		$class = get_called_class();
		$this->relating_key = (is_null($foreign_key)) ? strtolower(static::$class()).'_id' : $foreign_key;
		return $model::where('_id', array(
			'$in' => $this->attributes[$this->relating_key]
			));
	}
	...
	{
		return with(new static)->connection;
	}
