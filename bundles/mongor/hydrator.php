...
class Hydrator {
	...
	public static function hydrate($mongor, $results)
	{
		...
		if(count($models) > 0)
		{
			...
			foreach($mongor->includes as $include)
			{
				...
				// callsed class, array of results, relationship
	...
	private static function eagerly($mongor, &$parents, $include)
	{
		...
		elseif ($relating = $mongor->relating ==  'has_one_or_many')
		{
			if (count($parents) > 1) {
				static::has_many($relationship, $parents, $relating_key, $include, $_ids);
			} else {
				static::has_one($relationship, $parents, $relating_key, $include, $_ids);
			}
		}
		else
		{
